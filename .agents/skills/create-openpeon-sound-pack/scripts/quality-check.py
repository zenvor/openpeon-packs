...
def check_pack(pack_dir):
    ...
    if not os.path.isfile(manifest_path):
        return {"error": f"No openpeon.json in {pack_dir}", "verdict": "REJECTED"}
    ...
    if not os.path.isdir(sounds_dir):
        return {"pack_name": pack_name, "display_name": display_name, "error": "No sounds/ directory", "verdict": "REJECTED"}
    ...
    if not audio_files:
        return {"pack_name": pack_name, "display_name": display_name, "error": "No audio files in sounds/", "verdict": "REJECTED"}
...
def main():
    ...
    if args.from_index and args.pack_name:
        ...
        if not ok:
            print("Failed to download pack", file=sys.stderr)
            sys.exit(2)
